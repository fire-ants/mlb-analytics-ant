...
pitch17 <- select(tbl(my_db2017, "pitch"), gameday_link, num, des, type, tfs, tfs_zulu, id, sz_top, sz_bot, px, pz, pitch_type, count, zone)
...
for (mlbID in hitters) {
    ...
    TargetedAtBats16 <- filter(atbat16, batter == mlbID)
    TargetedAtBats17 <- filter(atbat17, batter == mlbID)
    
    # join filtered atbats to all pitches
    pitchesJoin16 <- collect(inner_join(pitch16, TargetedAtBats16))
    pitchesJoin17 <- collect(inner_join(pitch17, TargetedAtBats17))
    
    # score Qual and Quant mutate
    joined16 <- pitchesJoin16 %>% mutate(quant_score_des = get_quant_score(des),
    ...
    joined17 <- pitchesJoin17 %>% mutate(quant_score_des = get_quant_score(des),
                                         fix_quant_score = fix_quant_score(event) * (des == 'In play, run(s)'),
                                         quant_score = quant_score_des + fix_quant_score,
                                         qual_score = get_qual_score(atbat_des) * (type == 'X'),
                                         hitter_val = quant_score + qual_score)
    ...
    joined16$pitch_type <- as.factor(joined16$pitch_type) 
    joined16$des <- as.factor(joined16$des) 
    joined16$type <- as.factor(joined16$type)
    joined16$count <- as.factor(joined16$count) 
    joined16$event <- as.factor(joined16$event) 
    
    joined17$pitch_type <- as.factor(joined17$pitch_type) 
    joined17$des <- as.factor(joined17$des) 
    joined17$type <- as.factor(joined17$type)
    joined17$count <- as.factor(joined17$count) 
    joined17$event <- as.factor(joined17$event) 
    
    # convert FS and FT to SInkers 
    levels(joined16$pitch_type)[levels(joined16$pitch_type)=="FS"] <- "SI"
    levels(joined16$pitch_type)[levels(joined16$pitch_type)=="FT"] <- "SI"
    levels(joined16$pitch_type)[levels(joined16$pitch_type)=="FC"] <- "SL"
    levels(joined16$pitch_type)[levels(joined16$pitch_type)=="KC"] <- "KN"
    
    levels(joined17$pitch_type)[levels(joined17$pitch_type)=="FS"] <- "SI"
    levels(joined17$pitch_type)[levels(joined17$pitch_type)=="FT"] <- "SI"
    levels(joined17$pitch_type)[levels(joined17$pitch_type)=="FC"] <- "SL"
    levels(joined17$pitch_type)[levels(joined17$pitch_type)=="KC"] <- "KN"
    
    #subset for All Hits and AllBallsInPlay 
    #subAllHits <- subset(joined, type == "X" & des == "In play, no out" | des =="In play, run(s)")
    subAllBallsInPlay16 <- subset(joined16, type == "X")
